...
- 自动生成排行榜：总分、胜率、局数、均分
- 最近提交记录可删除
- 服务器模式下数据保存到 SQLite 数据库 `data/x2rank.db`；静态预览模式下暂存在浏览器 `localStorage`

## 服务器部署

推荐在服务器上用 Node 启动，这样所有战绩、评论、密码都会保存到服务器本地的 SQLite 数据库 `data/x2rank.db`。
...
`data/x2rank.db`、`data/store.json`、日志和备份目录都不会提交到 GitHub。旧版 `data/store.json` 如果存在，首次启动会自动迁移到 SQLite。
